...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["acao"])) {
    if ($_POST["acao"] == "incluir_acessorio") {
        $novoAcessorio = array(
            'nome' => $_POST['nome'],
            'preco' => $_POST['preco']
        );

        inserirDados('acessorios', $novoAcessorio);
    } elseif ($_POST["acao"] == "excluir_acessorio") {
        $idExcluir = $_POST['id'];
        excluirDados('acessorios', $idExcluir);
    } elseif ($_POST["acao"] == "atualizar_acessorio") {
        $idAtualizar = $_POST['id'];
        $acessorioAtual = obterRegistroPorId('acessorios', $idAtualizar);

        
    } elseif ($_POST["acao"] == "executar_atualizacao_acessorio") {
        $idAtualizar = $_POST['id'];
        $dadosAtualizados = array(
            'nome' => $_POST['nome_atualizado'],
            'preco' => $_POST['preco_atualizado']
        );

        atualizarDados('acessorios', $idAtualizar, $dadosAtualizados);
    }
}
...
<head>
    ...
    <title>Atualizar Acessórios</title>
...
<body class="container mt-5">
    ...
    <?php if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["acao"] == "atualizar_acessorio"): ?>

    <form method="post" action="" class="mb-4" id="formularioAtualizacao">
        <h3>Atualizar Acessório</h3>
        <input type="hidden" name="acao" value="executar_atualizacao_acessorio">
        <input type="hidden" name="id" value="<?php echo $acessorioAtual['id']; ?>">
        <div class="form-group">
            <label for="nome_atualizado">Nome:</label>
            <input type="text" name="nome_atualizado" value="<?php echo $acessorioAtual['nome']; ?>"
                class="form-control" required>
        </div>
        <div class="form-group">
            <label for="preco_atualizado">Preço:</label>
            <input type="text" name="preco_atualizado" value="<?php echo $acessorioAtual['preco']; ?>"
                class="form-control" required>
        </div>
    ...
    <div id="listProd">
        ...
        <ul class="list-group">
            ...
            <li class="list-group-item d-flex justify-content-between align-items-center">
                <div class="flex-fill">
                    <?php echo "{$acessorio['nome']} - R$ {$acessorio['preco']}"; ?>
                </div>
                <form method="post" action="" class="ml-2">
                    <input type="hidden" name="acao" value="excluir_acessorio">
                    <input type="hidden" name="id" value="<?php echo $acessorio['id']; ?>">
                    <button type="submit" class="btn btn-danger btn-sm">Excluir</button>
                </form>
                <form method="post" action="" class="ml-2">
                    <input type="hidden" name="acao" value="atualizar_acessorio">
                    <input type="hidden" name="id" value="<?php echo $acessorio['id']; ?>">
